...
function requestCustomRendererAction(actionId: string): void {
  const action = availableActions.value.find(candidate => candidate.id === actionId)
...
<template>
  ...
  >
    ...
    <CustomRendererSandbox
      ...
      @action="requestCustomRendererAction"
      @renderer-error="reportRendererError"
    />
