...
<template>
  ...
  <div v-else-if="textModeEnabled" class="agent-shell relative flex h-screen overflow-hidden bg-[#090b0d] p-4 text-gray-100">
    <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_22%_12%,rgba(80,227,230,0.14),transparent_34%),radial-gradient(circle_at_78%_20%,rgba(45,212,191,0.10),transparent_31%),linear-gradient(180deg,rgba(255,255,255,0.035),transparent_24%)]" />
    <div class="relative z-10 flex min-h-0 flex-1 flex-col gap-3">
      ...
      <div class="relative flex min-h-0 flex-1 overflow-hidden rounded-[30px] border border-cyan-200/16 bg-black/24 shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_28px_90px_rgba(0,0,0,0.48)] backdrop-blur-xl">
      <!-- Left: session sidebar (collapsible w-56/w-10) -->
      <AgentSessionSidebar />

      <!-- Center: compact manager runtime conversation -->
      <div class="min-w-0 flex-1 flex flex-col border-r border-cyan-200/10 bg-[#071012]/58">
        <AgentChatPanel />
      </div>
      ...
      >
        <!-- Collapse toggle -->
        <div class="flex h-14 items-center justify-start px-3 flex-shrink-0 border-b border-cyan-200/10 bg-white/[0.025]">
          <button
            class="rounded-full border border-cyan-200/14 p-2 text-cyan-100/50 transition-colors hover:bg-cyan-200/10 hover:text-white"
